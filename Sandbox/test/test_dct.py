...
class TEST2DDCT(unittest.TestCase):
    ...
    def setUp(self):
        ...
        x_cos1 = [np.cos((np.arange(N) + 0.5) * x * np.pi / N) for x in range(N)]
        ...
        y_cos1 = [np.cos((np.arange(N) + 0.5) * x * np.pi / N) for x in range(N-1, -1, -1)]
        self.x_cos2_list += [np.outer(x_cos, y_cos) for x_cos, y_cos in zip(x_cos1, y_cos1)]
        self.expected_cos2_list += [create_expected(n, m) for n, m in enumerate(range(N-1, -1, -1))]

    def test_dct2(self):
        for n, x in enumerate(self.x_cos2_list):
            y = dct2(x)
            np.testing.assert_allclose(y, self.expected_cos2_list[n], atol=1e-10)
